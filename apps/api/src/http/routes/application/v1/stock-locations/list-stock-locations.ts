...
const { FILTER_BY, SORT_BY, ORDER } = queries.stockLocations.listStockLocations

export async function listStockLocations(app: FastifyTypedInstance) {
  app.get(
    '/stock-locations',
    {
      schema: {
    ...
    async (request) => {
      ...
      const { count, stockLocations } =
        await tenant.queries.stockLocations.listStockLocations(
          { tenant: tenant.name },
